...
public class TimetableBody{
	...
	private String[][] tableBody; // Sun to Mon  , 00:01 to 24:00

	public void setData(List<TimeSlotData> days){
		
		String[][] table = new String[8][48];
	 
		// column array
		for(int i = 0; i < 7; i++) {
			String[] array = days.get(i).getSendTimeData().split(",");
			table[i+1] = array;
		
		}
		
		// add row header for each time slot
		String[] rowHeader = {
		...
		// Transpose
		String[][] transposedTable = new String[48][8];
		for(int i=0;i<48;i++) {
		   for(int j=0;j<8;j++) {
		...
		this.tableBody = transposedTable;
	}	
