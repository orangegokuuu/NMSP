...
//NumberFilter.CONTAINS = 'contains'; // 1;
NumberFilter.EQUALS = 'equals'; // 2;
NumberFilter.NOT_EQUALS = 'notEquals'; // 3;
NumberFilter.LESS_THAN = 'lessThan'; // 4;
...
NumberFilter.prototype.doesFilterPass = function(params) {
	...
	var value = this.filterParams.valueGetter(params.node);
	if (!value) {
		if (this.filterType === NumberFilter.NOT_EQUALS) {
			// if there is no value, but the filter type was 'not equals',
			// then it should pass, as a missing value is not equal whatever
			// the user is filtering on
			return true;
...
NumberFilter.prototype.createGui = function() {
	...
	this.eFilterTextField = this.eGui.querySelector("#filterText");
	this.eTypeSelect = this.eGui.querySelector("#filterType");
	this.errMsg = this.eGui.querySelector('#errMsg');
	this.eFilterTextField.addEventListener('change', this.onFilterChanged.bind(this));
    this.eTypeSelect.addEventListener("change", this.onTypeChanged.bind(this));
	this.setupApply();
};
...
NumberFilter.prototype.onFilterChanged = function() {
	...
	if (filterText !== null && filterText !== undefined) {
		try{
			newFilterText = parseInt(filterText); // parseInt
			if(isNaN(newFilterText)) { throw "is not a nubmer.";}
			if(newFilterText > 2147483647) { throw "too high.";}
			if(newFilterText < -1) { throw "too low.";}
			this.errMsg.innerHTML = "";
		} catch (err){
			// this.errMsg.innerHTML = "Input " + err ;
			this.errMsg.textContent = "Input " + err ;
		}
	} else {
		newFilterText = null;
	}
...
NumberFilter.prototype.getType = function() {
	return this.filterType;
};
