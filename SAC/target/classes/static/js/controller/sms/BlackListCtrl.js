SAC.controller('Sms.BlackListCtrl', function($scope, $rootScope, $http, $filter, $mdDialog, SmsService, AuthService, GridUtil,DateUtil) {
	...
	function header() {
		return [{ headerName : $translate('sms.bkl.field.01'), field : "destNumber", tooltipField : "key" },
		{ headerName : $translate("sms.bkl.field.02"), field : "createDate", tooltipField : "createDate", filter : 'date' },
		{ headerName : $translate("sms.bkl.field.03"), field : "createBy", tooltipField : "createBy" }];
	};
	...
	var dataSource = { getRows : function(params) {
		...
		SmsService.pageBlackList(searchPage).then(function(response) {
			...
			if (response.status=='200') {
				...
				pageResult.data.forEach(function(item) {
					if(item.createDate!=null){
						...
						var formattedDate = DateUtil.transDate(date, "yyyy/MM/dd hh:mm:ss");
	...
	$scope.exportCSV = function() {
		...
		SmsService.pageBlackList(searchPage).then(function(response) {
			if (response.status==200) {
				...
				alasql('SELECT destNumber, createDate, createBy INTO csv("blacklist.csv",{separator:","}) FROM ?', [pageResult.data]);
			}
		});
		$("#exportBtn").blur();
	}
