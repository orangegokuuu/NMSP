	...
	public void testCase1() throws JMSException, UnsupportedEncodingException {
		...
		for (int i = 0; i < 5; i++) {
			try {
				...
				producer.enqTextMsgA(new String(ByteUtil.hexStringToByteArray(MdpData.testData1[i][dataIndex])));
			} finally {
				if (producer != null) {
	...
	public void testCase26() throws JMSException, UnsupportedEncodingException {
		...
		for (int i = 0; i < 5; i++) {
			try {
				...
				producer.enqTextMsgAwithC(new String(ByteUtil.hexStringToByteArray(MdpData.testData2b[i][dataIndex])), new String(ByteUtil.hexStringToByteArray(MdpData.testData2b[i][dataIndex+1])));
			} finally {
				if (producer != null) {
					producer.closeConnection();
				}
